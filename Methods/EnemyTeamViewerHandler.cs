...
{
    public class EnemyTeamViewerHandler
    {
        ...
        public EnemyTeamViewerHandler(WrapPanel teamMemberHolder, Border memberTemplate, Run schoolRun, Run teamNameRun, Run currentGameRun, Run winCountRun, Run lossCount, Run winPercentRun, Image teamIcon)
        {
        ...
        {
            Team currentUserTeam = await EcacMethods.GetCurrentOpponent(GlobalGameData.Games.Valorant);
            ...
            foreach (User user in currentUserTeam.Members)
            {
                ...
                ValorantRank currentRank = await TrackerGg.GetCurrentRank(linkedHandle);
                ValorantRank peakRank = await TrackerGg.GetPeakRank(linkedHandle);

                user.SetRiotId(linkedHandle);
                user.SetValorantCurrentRank(currentRank);
                user.SetValorantPeakRank(peakRank);

                Border template = MemberTemplate.Dispatcher.Invoke(() => MemberTemplate.CloneControl());
                ...
                kdaRun!.Text = await TrackerGg.GetKdRatio(linkedHandle);
                hsRun!.Text = await TrackerGg.GetHeadshotPercentage(linkedHandle);
                ...
                thirdImage.Source = new BitmapImage(new Uri(user.ValorantPeakRank.RankIcon));
                ...
                Console.WriteLine(@"Finished Enemy: " + user.EcacName);
            }
